...
function scroll_scroll() {
	...
	if (scr_blocks.length > 0) {
		for (let index = 0; index < scr_blocks.length; index++) {
			...
			if (
				...
				pageYOffset <
					block_offset + block_height - window.innerHeight / 5
			) {
	...
	if (custom_scroll_line) {
		...
		let scr_procent =
			(pageYOffset / (content_height - window_height)) * 100;
		...
		if (scrolling_full) {
			...
			if (
				...
				if (current_block_prev) {
					let current_block_prev_height =
						current_block_prev.offsetHeight;
...
if (link) {
	...
	window.addEventListener('scroll', function (el) {
		let old_current_link = document.querySelectorAll(
			'._goto-block._active'
		);
		if (old_current_link) {
		...
		for (let index = 0; index < blocks.length; index++) {
			...
			if (block_item) {
				...
				if (
					...
					pageYOffset <
						block_offset + block_height - window.innerHeight / 3
				) {
...
if (goto_links) {
	for (let index = 0; index < goto_links.length; index++) {
		...
		goto_link.addEventListener('click', function (e) {
			let target_block_class = goto_link
				.getAttribute('href')
				.replace('#', '');
			let target_block = document.querySelector('.' + target_block_class);
			_goto(target_block, 300);
...
function custom_scroll(event) {
	...
	let custom_cursor_height = Math.min(
		window_height,
		Math.round(
			window_height * (window_height / custom_scroll_content_height)
		)
	);
